 ...
 */
 ...
 function b2w_entry_footer() {
 	...
 	if ( 'post' === get_post_type() ) {
 		...
 		if ( $categories_list && b2w_categorized_blog() ) {
  ...
  * @return bool
  */
 function b2w_categorized_blog() {
 	if ( false === ( $all_the_cool_cats = get_transient( 'b2w_categories' ) ) ) {
 		...
 		set_transient( 'b2w_categories', $all_the_cool_cats );
 	}

 	if ( $all_the_cool_cats > 1 ) {
 		// This blog has more than 1 category so b2w_categorized_blog should return true.
 		return true;
 	} else {
 		// This blog has only 1 category so b2w_categorized_blog should return false.
 		return false;
 	}
 }

 /**
  * Flush out the transients used in b2w_categorized_blog.
  */
 function b2w_category_transient_flusher() {
 	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
 		return;
 	}
 	// Like, beat it. Dig?
 	delete_transient( 'b2w_categories' );
 }
 add_action( 'edit_category', 'b2w_category_transient_flusher' );
 add_action( 'save_post',     'b2w_category_transient_flusher' );
  ...
  */
 function b2w_comment_list( $comment, $args, $depth ) {
  ...
  */
 function b2w_numbered_pagination() {
 	global $wp_query;
 	$big = 999999999;
